        ...
        case (GAPM_PROFILE_TASK_ADD):
        {
            ...
            if (!appm_add_svc())
            {
                ...
                ke_state_set(TASK_APP, APPM_READY);
				 ...
				 /*BELON @Emily 上电2s后进入深度休眠 ,休眠电流1.4uA，P3.1下降沿中断唤醒，之后继续休眠，step 6*/
				 #if 1 
							
					ke_timer_set(APP_SCANTO_TIMER,TASK_APP,200);	
...
/*BELON @Emily 上电2s后进入深度休眠 ,休眠电流1.4uA，P3.1下降沿中断唤醒，之后继续休眠，step 1*/
#if  1
...
uint8_t key_wakeup_flag;
static int app_scanTO_timer_handler(ke_msg_id_t const msgid,
                                    void const *param,
                                    ke_task_id_t const dest_id,
                                    ke_task_id_t const src_id)
{
	UART_PRINTF("%s\r\n",__func__);
       if(key_wakeup_flag == 0)
        {
...
const struct ke_msg_handler appm_default_state[] =
{
    ...
    {APP_PERIOD_TIMER,				(ke_msg_func_t)app_period_timer_handler},
		{APP_SCANTO_TIMER,         (ke_msg_func_t)app_scanTO_timer_handler}
};
